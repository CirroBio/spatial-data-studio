...
3. **One contract envelope.** Every function returns `CallResult`
   (`backend/app/registry/base.py`): `{status, log, structural_diff,
   changed_fields, changed_facets, figure_svg?, figure_pdf?, new_object?,
   error?}`. (R5)
