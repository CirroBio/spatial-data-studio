...
| R6/R7 | Compute history is append-only; never `COMPLETED→QUEUED`; rerun appends. | v1 | `test_invariants.py::test_r6_r7_*` — static, and **partial**: the no-`COMPLETED→QUEUED` and rerun-appends halves only. Whether a record is ever *removed* is review-only, and needs a run harness (`config.SYNTH_FIXTURE`) |
