"""Snapshots: a `<name>.sview.json` view config (viewport + display encoding) over an
immutable checkpoint, saved alongside it in DATA_DIR. Opening a snapshot
(`SessionManager.create_from_snapshot`) loads its referenced checkpoint as a read-only,
in-app session pinned to the saved view — the same way any other checkpoint opens, just
read-only and with a pinned display instead of the auto-generated default. There is no
standalone viewer: a snapshot is only viewable through this running app, not shipped as
an independent static page. Because the pinned display is restored from the saved
`encoding` verbatim, every per-channel setting the encoding carries — visibility, name,
custom color, contrast min/max — reproduces exactly as it looked live.

Saving a snapshot ensures the session is first written to a content-hashed checkpoint (so
the config points at bytes that won't change under it) under a read lock, so no compute
can interleave between the save and capturing the view.
"""
...
# shape changes, so a raw file on disk shows which version wrote it. 2.1 dropped the
# legacy baked `render` manifest — the in-app viewer restores from `encoding` directly.
SNAPSHOT_CONFIG_VERSION = "2.1"
