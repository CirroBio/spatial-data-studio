...
RAM (`SDS_WORK_DIR` defaults to `/work` in the image). That tmpfs usage is folded into
the admission accounting so the soft 503 boundary trips before the tmpfs can grow the
container past `--memory` into an OOM kill — read from the container's own cgroup
(`shmem`), so it is counted whether or not `SDS_WORK_DIR_IN_RAM` is set.
`--cap-add SYS_ADMIN` lets the
...
| `SDS_WORK_DIR_IN_RAM`    | `0` (compose: `1`) | Only consulted **outside** a memory-limited container, where the tmpfs working set is estimated by `os.statvfs`: set it to `1` when `SDS_WORK_DIR` is a **dedicated** tmpfs mount so that usage is added to RSS in the admission/boundary math, and leave `0` when `WORK_DIR` is on disk. In a container the same figure is read from the cgroup (`shmem`) regardless of this flag. |
...
| `SDS_CONTAINER_CPUS`     | auto (cgroup, else host cores) | CPU cores the app may use — the single knob that sizes the compute pool and every thread budget below, and the denominator the resource strip shows CPU usage against. **Unset: auto-detected from the cgroup CFS quota** (an ECS task `cpu` / Fargate `cpu` / `docker --cpus`), falling back to the host's `os.cpu_count()` when the container has no CPU hard-limit. Set it to override — e.g. when only an ECS *container-level* `cpu` share (not a task-level hard cap) is configured, which the app can't read as a core count. Fractional values are allowed. |
| `SDS_MAX_SESSIONS`       | `8`       | Maximum concurrent in-memory sessions. |
| `SDS_IMAGE_RENDER_CONCURRENCY` | auto (CPU alloc) | Max image tiles/thumbnails composited at once. Caps the transient memory of a zoom/pan tile burst; renders past `SDS_ADMISSION_PCT` return 503 and the canvas keeps its coarse base layer. Scales with the CPU allocation (`SDS_CONTAINER_CPUS`) by default — set lower on a memory-constrained container. |
...
| `SDS_RASTER_REBUILD_WORKERS` | auto (CPU alloc) | dask worker count for the one-time ingest re-tiling. Scales with the CPU allocation (`SDS_CONTAINER_CPUS`) by default — set lower on a memory-constrained container. |
